...
## 2026-07-26 — Renderer control and analysis complete

The strict `S1-G1` through `S5-G5` sequence completed with 22 measured active tests and verified renderers in every run. SwiftShader was red 5/5 with 31 failures; native GPU was green 5/5 with zero failures. Each predeclared Chromium core combination moved 5/5→0/5. Median end-to-end duration moved 211.287 s→50.824 s, median max CPU PSI 97.37%→45.18%, median max one-minute load 152.52→71.44, and median max runnable processes 228→66. Firefox stayed on the RTX 3080 in both arms, yet its eight SwiftShader-arm failures disappeared when Chromium moved off SwiftShader, supporting whole-host renderer-load spillover.

Post-run trace analysis preserves the split branches. Across the ten accepted instrumented SwiftShader runs, A produced 22 camera-distance-unchanged surfaces, two later drag-action timeouts, and one unresolved timeout; B split 6 Resume versus 4 Reset actionability hangs; C was 10/10 second-navigation load stalls after a fast HTTP 200 response (7 traces without recorded receipt completion, 3 complete bodies without `load`); D split 7 controls-enabled-at-first-read versus 2 graph-handle-unavailable occurrences. H2 survives at the layered-family level and H3 is strongly supported, but exact internal mechanisms remain unresolved.
