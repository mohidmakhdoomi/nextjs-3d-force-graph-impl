### Iteration 1 Reviews
- gemini: REQUEST_CHANGES — The H1 fix is implemented perfectly, but the canonical suite retains heavyweight verbose pointer event logs in violation of FR2's trim rules.
- codex: REQUEST_CHANGES — The H1 fix in `matrix.spec.ts` is well-targeted, but Phase 3 is not complete because heavyweight Phase-1/2 diagnostics are still committed in the canonical `tests/e2e` harness.
- claude: APPROVE — Phase 3 delivers the evidence-selected H1 fix (probe-verified background start point with edge-clearance margin) — minimal, behavior-preserving, correctly scoped, and well-documented.
